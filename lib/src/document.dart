...
import 'package:jv_easy_pdf_viewer/src/page.dart';
...
class PDFDocument {
  ...
  static Future<PDFDocument> fromURL(
    ...
    // Download into cache
    File f = await (cacheManager ?? DefaultCacheManager()).getSingleFile(
      url,
      headers: headers,
    );
    PDFDocument document = PDFDocument();
  ...
  static void fromURLWithDownloadProgress(
    ...
    final fileResponse = (cacheManager ?? DefaultCacheManager()).getFileStream(
      url,
      headers: headers,
      withProgress: true,
    );

    streamSubscription = fileResponse.listen((event) async {
      ...
      if (event is FileInfo) {
        final pdfDocument = await fromFile(
          event.file,
  ...
  Future<PDFPage> get({
    ...
    var data = await _channel.invokeMethod('getPage', {
      'filePath': _filePath,
      'pageNumber': page,
    });
    return PDFPage(
